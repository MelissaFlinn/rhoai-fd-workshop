...
. Set the value to `data/card_transdata.csv` which contains the data to train your model, and then click *Add*.
+
...
In node 1, the notebook creates the `models/fraud/model.onnx` file. In node 2, the notebook uploads that file to the S3 storage bucket. You must set `models/fraud/model.onnx` file as a dependency for both nodes.
...
You must set the S3 storage bucket keys by using the secret created by the `My Storage` data connection that you set up in the xref:1-03-data-connections.adoc[Storing data with data connections] section of this {deliverable}.
...
The result should be a `models/fraud/model.onnx` file in your S3 bucket which you can serve, just like you did manually in the xref:3-01-prepare-model.adoc[ Preparing a data model for deployment] section.
