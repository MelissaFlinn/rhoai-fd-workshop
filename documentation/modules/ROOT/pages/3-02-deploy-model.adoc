= 2.2 Deploying the data model

include::_attributes.adoc[]

Now that the data model is accessible in storage and saved in a portable `onnx` format, you can use an OpenShift Data Science model server to deploy it as an API.

OpenShift Data Science multi-model servers can host several models at once.  You create a new model server and deploy your model to it.

.Procedure

. In the OpenShift Data Science dashboard, navigate to *Models and model servers*.
. Click *Add server*.
+
image::model-serving/ds-project-model-list-add.png[Add Server]

. In the form:

.. Fill out the *Model server name*, for example `Model Server`.
.. Select `OpenVINO Model Server`.
.. Leave the other fields (replicas, size, route, auth) with their default settings.
+
image::model-serving/create-model-server-form.png[Add Server Form]
//missing a step - click create?

. In the *Models and model servers* list, next to the new model server, click *Deploy model*.
+
image::model-serving/ds-project-workbench-list-deploy.png[Deploy model button]

. In the form:

.. Fill out the *Model Name* with the value `fraud`.
.. Select the server that you created (for example, `Model Server`).
.. Select the *Existing data connection*: `My Storage`
.. Enter the path to your uploaded model: `models/fraud/model.onnx`
+
image::model-serving/deploy-model-form.png[Deploy model form]

. Wait for the model to deploy and for the *Status* to show a green checkmark.
+
image::model-serving/ds-project-model-list-status.png[Model status]

. Take note of the API Endpoint name and URL. You need this information in the next section when you test the model.

image::model-serving/ds-project-model-list-name.png[Model name]

image::model-serving/ds-project-model-list-url.png[Model URL]

.Next step

Testing the model