= 2.1 Creating a workbench and selecting a Notebook image
...
* You created a `My Storage` data connection xref:1-03-data-connections.adoc[as described in section 1.3].
...
+
Red Hat provides several supported notebook images. In the *Notebook image* section, you can choose one of these images or any custom images that an administrator has set up for you. The *Tensorflow* image which has the necessary libraries for this {deliverable}.

. Choose the latest *Tensorflow* image.
+
image::workbenches/create-workbench-form-image.png[Workbench image, 600]

. Choose a small deployment and no GPUs. This {deliverable} does not require GPUs.
+
...
NOTE: If you have made a mistake, you can stop the workbench and edit it to make changes.
...
xref:2-02-jupyter-env.adoc[Importing the {deliverable} files into the Jupyter environment]
